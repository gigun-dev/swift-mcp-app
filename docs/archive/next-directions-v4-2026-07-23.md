...
- **履歴カード:** 会話本文はread-onlyのまま、同一serverをprovenanceで確定できるカードだけlive islandとして
  現在のUI resourceへ再接続する。保存resultへnamespaced revalidation hintを載せ、App自身の最初のrefresh成功まで
  操作をgateする。Swift hostは元toolやrefresh名を推測・自動実行せず、未対応・失敗・timeoutはfail-closedする。
- **検証基盤:** `make check`はKernel/Servicesのbuild・215 tests / 23 suites・baselineなしlint、
...
- **横操作の所有権:** 閉状態のchat paneは右swipeでdrawerを開くが、開始点が現在表示中MCP Appの
  縦帯ならhostは一切追従せずWKWebViewへ譲る。開状態はsidebar内の左swipe、または露出main cardの
  tap / 左swipeで閉じる。露出mainのtapとdragは単一gestureで排他し、二重確定によるガクつきを防ぐ。
  履歴管理は標準context menu（pin・rename・確認付きdelete）へ集約する。
...
- [ ] todos/agendaが履歴revalidation hintを受けたらstaleTimeを迂回して各`refresh-*`を1回呼び、
  Swift hostの操作gateが成功後だけ解除されることを実装・専用Simulatorで確認する。
