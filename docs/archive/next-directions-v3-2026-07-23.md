...
- **検証基盤:** `make check`はSwift build/test/lintの高速gate、`make app`はSwiftUIを含むiOS全体build、
  ...
  baselineなしSwiftLint strict 0件、201 tests / 21 suites、generic iOS Simulator app buildまでgreen。
...
- **次の着手順:** ①tool pickerの4 UX判断を合意後に実装・回帰 ②承認後にD blindと固定版Bを実行し、
  差が出た試験だけCを診断 ③caldav本番への一時書込と標準Calendar/Reminders account追加を承認後に
  agenda/open-link/保存往復を消化。Claude Code parityはrate limit解消後に行う。harnessの採用判断なしに
  現行fallbackを削除しない。
