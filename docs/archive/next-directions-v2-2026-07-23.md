...
- **検証済み**: `make check`はSwift build/test/lintを通す統合完了ゲート。静的解析だけの再実行用に
  `make lint`も持つ。SwiftLint baselineは撤去し、SwiftFormatは意図コメントを壊さない字句ルールに
  限定しつつ、Sources/Testsの既存違反もコード側で解消して0件を維持する。
...
- [x] ~~**dotfiles の nix に `xcodegen` / `swiftformat` / `swiftlint` を追加する**~~ ✅
      ...
      SwiftFormatはコメント方針と両立する安全な字句ルールに限定。`make lint`はツール不在を
      skipせず失敗し、SwiftLint baselineなしでSources/Testsの全違反を検出する。
      `make check`はbuild/test後に`make lint`も呼ぶ統合完了ゲート。各工程は個別実行もできる。
