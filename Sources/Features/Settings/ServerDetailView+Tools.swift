// ServerDetailView のツール許可インライン(design/09 S2・「有効/状態」と同階層のツール一覧)。
//
// 2026-07-24 階層を1段浅く(ユーザーFB): 以前は独立した中間プッシュ画面 ServerToolPermissionsView
// (「ツール」→ 一覧)だったが、claude.ai のコネクタ詳細と同じく「有効/状態」と同じ Form 内に
// ツール一覧が並ぶ形にすべき、との指摘を受けて中身を ServerDetailView へインライン化した。
// トップレベル View は廃止し、一覧・一括・行・詳細読み込みロジックを ServerDetailView の extension
// としてこのファイルに残す(ServerDetailView 本体の type_body_length を膨らませないための分割)。
// 挙動は不変 —— refreshToken 反映方式・store(ToolPermissionStore)・集約状態算出・trusted:true 固定・
// 決定キー=生ツール名は旧 ServerToolPermissionsView からそのまま移植した(階層だけ浅くする)。
//
// 役割: サーバーが接続中(.ready)なら tools/list を名前順に並べ、各行に「表示名 + 状態ラベル +
// chevron」を出す。行タップで ToolPermissionDetailView(画面3)へ push。行内に切替コントロールは
// 置かない —— モバイル幅ではツール名の表示幅を優先する(design/09 決定1)。
//
...
extension ServerDetailView {
    ...
    /// ServerDetailView の Form に差し込むツール許可セクション群。接続状態は home 経由で観測し、
    /// .ready のときだけツール行 + 一括メニューを出す(未接続時はプレースホルダのみ)。
    @ViewBuilder
    func toolPermissionsContent(_ state: ConnectionsManager.State) -> some View {
        if case .ready(let ready) = state {
            toolsSection(ready)
        } else {
            // 未接続(.ready でない)サーバーは tools を列挙できない。権限行は出さず「接続すると
            // 設定できる」旨だけ示す(design/09・タスク指示のプレースホルダ)。状態自体は上の
            // connectionSection が既に出しているので、ここでは重複させず案内文だけに絞る。
            placeholderSection()
        }
    }
    ...
    private func toolsSection(_ ready: ReadyConnection) -> some View {
        ...
        return Section {
            ForEach(Array(sorted.enumerated()), id: \.offset) { _, tool in
                ...
                // pop で戻るたびに行ラベルを取り直す(toolRefreshToken コメント参照)。旧
                // ServerToolPermissionsView は画面全体の onAppear で進めていたが、インライン化で
                // 画面 onAppear は詳細 pop 以外(サーバー詳細を開いた瞬間)でも発火してしまうため、
                // ツール行側の onAppear に載せ替えて「詳細から戻ったとき」により近づける。
                .onAppear { toolRefreshToken &+= 1 }
            }
        } header: {
            // 見出し + 一括メニュー(design/09 画面2「一括メニュー」・ユーザーFB 2026-07-24)。
            // remote MCP をコネクタ単位でまとめて許可/確認/ブロック/既定へ戻す UI。
    ...
    /// (Kernel/Services の新規 API は不要・タスク指示)。適用後は toolRefreshToken を進めて一覧を
    /// 再評価させ、各行ラベルを即更新する(storedDecision を読み直す・既存の反映方式に一括適用も接続)。
    @ViewBuilder
    private func bulkMenuItems(tools: [Tool]) -> some View {
        // 「すべてのツールを常に許可」= 全ツールに .allow を明示保存(確認を出さず即実行)。
        Button {
            applyBulk(tools: tools) { toolStore.setDecision(.allow, serverURL: entry.url, toolName: $0) }
        } label: {
        ...
        Button {
            applyBulk(tools: tools) { toolStore.setDecision(.ask, serverURL: entry.url, toolName: $0) }
        } label: {
            Label("すべてのツールを確認する", systemImage: "hand.raised")
        }
        // 「すべてのツールをブロック」= 全ツールに .deny を保存(モデル発でもカード発でもハードブロック)。
        Button(role: .destructive) {
            applyBulk(tools: tools) { toolStore.setDecision(.deny, serverURL: entry.url, toolName: $0) }
        } label: {
        ...
        Button {
            applyBulk(tools: tools) { toolStore.clearDecision(serverURL: entry.url, toolName: $0) }
        } label: {
            Label("既定に戻す(自動)", systemImage: "arrow.uturn.backward")
        }
    }

    /// 一括適用の共通処理。全ツールに `apply`(生ツール名を渡す)を実行し、最後に toolRefreshToken を
    ...
    /// 「stored ?? defaultDecision」式で実効決定を組むので、表示と実挙動がズレない(冒頭の写像)。
    private func aggregateLabel(tools: [Tool]) -> String {
        _ = toolRefreshToken  // 依存注入(一括適用後の再評価トリガ・statusLabel と同じ理由)。
    ...
    private func effectiveDecision(for tool: Tool) -> ToolPermissionDecision {
        if let stored = toolStore.storedDecision(serverURL: entry.url, toolName: tool.name) { return stored }
        let annotations = kernelAnnotations(from: tool.annotations)
        // trusted:true 固定(冒頭の根拠・runtime ゲートと同じ注入)。
    ...
    private func placeholderSection() -> some View {
        Section {
            ...
            Text("接続するとツールの権限を設定できます。上の「有効」を入れてサーバーを認証してください。")
        }
    }
    ...
    private func toolRow(_ tool: Tool) -> some View {
        ...
        // app 専用ツールは LLM からは隠れているが、カード発の tools/call で呼ばれうるため許可設定
        // 自体は残す(表示も残す)。metadata 変換失敗時は理由なく app 専用扱いしないよう
        // model-visible 側へ倒す。
    ...
    /// 状態ラベル(corrected モデル: stored ?? default)。冒頭コメントの写像に一致させる。
    @ViewBuilder
    private func statusLabel(for tool: Tool) -> some View {
        ...
        if let stored {
            ...
            // 未保存 → annotations 由来の既定表示。trusted:true 固定(冒頭の根拠)。
            let annotations = kernelAnnotations(from: tool.annotations)
    ...
    /// 生の永続値を読む薄いラッパ。body 評価中に toolRefreshToken(@State)を読むことで SwiftUI の
    /// 依存を張り、詳細から pop で戻って onAppear が toolRefreshToken を進めたとき、この行が再評価
    /// されて storedDecision を読み直す(ToolPermissionStore は @Observable でないための最小反映手段)。
    private func storedDecision(for tool: Tool) -> ToolPermissionDecision? {
        _ = toolRefreshToken  // 依存注入(値は使わない・再評価トリガのため)。
        return toolStore.storedDecision(serverURL: entry.url, toolName: tool.name)
    }
