...
struct HapticThrottleTests {
    ...
    func withinIntervalSuppressed() {
        ...
        #expect(throttle.shouldFire(now: baseline) == true)
        #expect(throttle.shouldFire(now: baseline.addingTimeInterval(0.05)) == false)
    }
    ...
    func afterIntervalFiresAgain() {
        ...
        let baseline = Date(timeIntervalSince1970: 0)
        #expect(throttle.shouldFire(now: baseline) == true)
        #expect(throttle.shouldFire(now: baseline.addingTimeInterval(0.1)) == true)
    }
